    ...
    $('form').submit(function () {
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result == false)
                    err_str.push('<strong> Mật khẩu hiện tại không chính xác </strong>');
            }
        });
        if (pass.length < 8)
            err_str.push("Mật khẩu phải có ít nhất 8 ký tự");
        if (!(/\S\d|\d\S/).test(pass))
            err_str.push("Phải bao gồm cả ký tự thường và ký tự số");
        if (pass != repass)
            err_str.push("Mật khẩu không khớp");
        if (!err_str.length)
        {
