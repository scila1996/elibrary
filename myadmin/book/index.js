$(document).ready(function () {
    ...
    function duplicate_Code(code, exclude_id)
    {
        var r;
        ...
        $.ajax({
            ...
            async: false,
            data: {action: "check-duplicate-code-book", exclude_id: exclude_id, code: code},
            dataType: "json"
        }).done(function (result) {
            r = result;
        });
    ...
    $(document).on('submit', '#edit-book', function () {
        ...
        if (!duplicate_Code(book.code, book.id))
        {
            $.ajax({
                type: "POST",
                url: "/module/action.php",
        ...
        {
            alert('Mã sách bị trùng với sách khác ! Vui lòng kiểm tra lại');
        }
        return false;
    });
    // Select Book
