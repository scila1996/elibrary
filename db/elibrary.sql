...
--
-- Host: 127.0.0.1
-- Generation Time: Jun 28, 2016 at 06:00 PM
...
CREATE TABLE `admins` (
  `tên quản trị viên` char(50) NOT NULL DEFAULT 'Quản trị viên' COMMENT 'Tên người quản trị',
  `tài khoản` char(30) NOT NULL COMMENT 'Tài khoản người quản trị',
  `mật khẩu` char(100) NOT NULL DEFAULT 'd033e22ae348aeb5660fc2140aec35850c4da997',
  `thời gian hoạt động` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP
...
INSERT INTO `admins` (`tên quản trị viên`, `tài khoản`, `mật khẩu`, `thời gian hoạt động`) VALUES
('Nguyễn Trung', 'root', '9c7a31a8336cabd3298dbfef065214245e4a4176', '2016-06-28 16:52:51');
...
CREATE TABLE `books` (
  `id` char(50) NOT NULL,
  `mã danh mục` char(50) NOT NULL,
  `ngày thêm` date NOT NULL,
  `tiêu đề` varchar(1024) NOT NULL,
  `mã sách` char(50) NOT NULL,
  `tác giả` char(255) NOT NULL,
  `giá` int(11) NOT NULL,
  `nhà xuất bản` char(255) NOT NULL,
  `năm xuất bản` int(11) NOT NULL,
  `mô tả` text NOT NULL,
  `số lượng` int(11) NOT NULL,
  `số trang` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
INSERT INTO `books` (`id`, `mã danh mục`, `ngày thêm`, `tiêu đề`, `mã sách`, `tác giả`, `giá`, `nhà xuất bản`, `năm xuất bản`, `mô tả`, `số lượng`, `số trang`) VALUES
...
('0d467f764fc7ec8cdf483e71389ae1eb', '816be864bf1ff2e4d04b154cc43cd8c7', '2016-06-10', 'Chế độ pháp lý về quản lý tài chính và cổ phần hóa trong doanh nghiệp', 'ac6a76f2', 'Trần, Xuân Kiêm', 20000, 'Quốc gia', 2000, 'Không có mô tả', 8, 603),
...
CREATE TABLE `categories` (
  `mã danh mục` char(50) NOT NULL,
  `tên danh mục` char(255) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
INSERT INTO `categories` (`mã danh mục`, `tên danh mục`) VALUES
...
CREATE TABLE `issuedetails` (
  `mã` char(50) NOT NULL,
  `mã mượn` char(50) NOT NULL,
  `mã sách` char(50) NOT NULL,
  `tình trạng` tinyint(4) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
INSERT INTO `issuedetails` (`mã`, `mã mượn`, `mã sách`, `tình trạng`) VALUES
...
CREATE TABLE `issues` (
  `mã` char(50) NOT NULL,
  `tên người dùng` char(30) NOT NULL,
  `ngày mượn` datetime NOT NULL,
  `ngày phải trả` date NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
INSERT INTO `issues` (`mã`, `tên người dùng`, `ngày mượn`, `ngày phải trả`) VALUES
...
CREATE TABLE `issue_rule` (
  ...
  `tổng số sách có thể mượn` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
INSERT INTO `issue_rule` (`tháng`, `tổng số sách có thể mượn`) VALUES
(6, 5);
...
CREATE TABLE `users` (
  `tài khoản` char(30) NOT NULL,
  `mật khẩu` char(100) NOT NULL DEFAULT 'SHA1(1234561)',
  `tên người dùng` char(50) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
INSERT INTO `users` (`tài khoản`, `mật khẩu`, `tên người dùng`) VALUES
...
ALTER TABLE `books`
  ...
  ADD UNIQUE KEY `code` (`mã sách`),
  ADD KEY `categoryid` (`mã danh mục`);

--
-- Indexes for table `categories`
--
ALTER TABLE `categories`
  ADD PRIMARY KEY (`mã danh mục`),
  ADD UNIQUE KEY `categoryid` (`mã danh mục`);
...
ALTER TABLE `issuedetails`
  ADD PRIMARY KEY (`mã`),
  ADD KEY `issueid` (`mã mượn`),
  ADD KEY `bookid` (`mã sách`);
...
ALTER TABLE `issues`
  ADD PRIMARY KEY (`mã`),
  ADD UNIQUE KEY `id` (`mã`),
  ADD KEY `user` (`tên người dùng`);

--
-- Indexes for table `users`
--
ALTER TABLE `users`
  ADD PRIMARY KEY (`tài khoản`),
  ADD UNIQUE KEY `user` (`tài khoản`);
...
--
ALTER TABLE `books`
  ADD CONSTRAINT `books_ibfk_1` FOREIGN KEY (`mã danh mục`) REFERENCES `categories` (`mã danh mục`);

--
-- Constraints for table `issuedetails`
--
ALTER TABLE `issuedetails`
  ADD CONSTRAINT `issuedetails_ibfk_1` FOREIGN KEY (`mã mượn`) REFERENCES `issues` (`mã`),
  ADD CONSTRAINT `issuedetails_ibfk_2` FOREIGN KEY (`mã sách`) REFERENCES `books` (`id`);

--
-- Constraints for table `issues`
--
ALTER TABLE `issues`
  ADD CONSTRAINT `issues_ibfk_1` FOREIGN KEY (`tên người dùng`) REFERENCES `users` (`tài khoản`);
